...
@section('store_route', route('testimonials.update', $data))
                ...
                @foreach (config('translatable.locales') as $key => $locale)
                    <div class="tab-pane fade show @if ($key == 0) active @endif" id="{{ $locale }}"
                        ...
                        <div class="form-group">
                            <label>@lang('testimonials.title') - @lang('general.'.$locale)<span class="text-danger"> *
                                </span></label>
                            ...
                            <label>@lang('testimonials.job_title') - @lang('general.'.$locale)<span class="text-danger"> *
                                </span></label>
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text"><i class="flaticon-edit"></i></span>
                                </div>
                                <input type="text" name="{{ $locale . '[job_title]' }}" id="{{ $locale . '[job_title]' }}"
                                    placeholder="@lang('categories.job_title')"
                                    class="form-control  pl-5 min-h-40px @error($locale . '.job_title') is-invalid @enderror"
                                    value="{{ old($locale . '.job_title', $data->translate($locale)->job_title) }}">
                            </div>
