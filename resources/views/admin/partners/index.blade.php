...
@section('module_name') @lang('partners.partners') @endsection
@section('page_type') @lang('general.index') @endsection
@section('title') @lang('partners.partners') @endsection
@section('create_route', route('partners.create'))
        ...
        <tbody>
            @foreach ($data as $key => $value)
                <tr>
                    ...
                    <td><img src="{{ asset($value->main_image) }}" style="width: 200px;height:200px;"></td>
                    <td class="sorting_1" nowrap="nowrap">
                        <span class="label-inline">{{ $value->created_at }}</span>
                    </td>
                    <td nowrap="nowrap">
                        @include('admin.components.table-control', ['name'=>'partners', 'value'=>$value])
